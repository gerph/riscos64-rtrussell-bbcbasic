...
.global _kernel_swi_flags

// _kernel_oserror *_kernel_swi_flags(int swinum, int *inregs, int *outregs, flagsp)
// If b31 of swinum is set, we clear the X bit.
    FUNC    "_kernel_swi_flags"
    STP     x29, x30, [sp, #-16]!
    MOV     x29, sp
    MOV     x13, x3
    MOV     x4, x0, LSR #31-17
...
_kernel_swi_out_regs_done:
    ...
    CMP     x13, #0
    BEQ     _kernel_swi_out_flags_done

    STR     w14, [x13]

_kernel_swi_out_flags_done:
    LDP     x29, x30, [sp], #16
    RET
